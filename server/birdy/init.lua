...
-- Boot: create/upgrade the phone_birdy_* tables before any callback can touch them. One-shot;
-- pcall so a SQL outage degrades to a logged error instead of killing the resource.
...
---Push an event to a single player. No-op when they're offline - live pings are best-effort;
---the DB rows are the durable copy the next open reads.
...
---Forward an action result, pinging the `notifyCid` player (if online) with a notification
---event, then stripping that internal field so the caller's NUI never receives another player's
---citizenid it didn't already know. The push targets exactly one resolved source - never a
---broadcast.
---@param result table
---@return table
...
-- Authoritative app callbacks: thin delegates into server.birdy.actions, which owns the
-- validation + mutation (each handler is documented there). The actor is always re-derived from
-- src inside actions - payloads never carry identity.
...
---Send a DM: deliver the recipient's copy as a live push when they're online, then rewrite the
---envelope so the sender only receives their own message - the routing fields (toCid / fromCid /
---fromProfile / messageForOther) stay server-side. The push goes to exactly one resolved source.
...
---Toggle a DM reaction: push the other participant their per-viewer reaction set (their convo is
---keyed by the caller's cid), then rewrite the envelope so the caller only receives their own
---view - the otherCid / otherReactions routing fields stay server-side.
---@param src number player server id
