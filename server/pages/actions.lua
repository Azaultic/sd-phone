...
---{ success, message?, data? } envelope. Posts are server-authoritative: the owner is the
---caller's citizenid and the timestamp is set here, so clients can't spoof authorship or dates.
---Structural twin of server.marketplace.actions - Pages is the priceless "yellow pages" board.
local actions = {}
...
---DB row -> the shape the React app renders. `image` stays nil (and so drops out of the JSON)
---when unset, which the UI treats as no photo. Pages is a yellow-pages board, not a marketplace,
---so a price is never exposed - the field simply isn't in the shape. The owner's citizenid is
---never sent: authorship is exposed only as the `mine` boolean, computed against the viewer's
---cid (nil for broadcast copies, so no recipient of a feed push is ever marked as the author).
---@param row table post DB row
...
---Validate + normalise a post payload into the columns we store: the fields table, or nil + an
---error message for a missing title/description/contact. Shared by create and update so both
...
--- - price: always nil - Pages has no marketplace pricing; the column exists only for schema
---   parity with marketplace_listings.
...
---   post only advertises the contact methods the poster actually attached.
--- - email: trimmed + lowercased, capped to the column's 128 chars, nil when blank.
---A post must carry at least one way to reach the poster (number or email).
...
---Push a live feed change to every OTHER player so anyone with Pages open sees it without
...
---Create a post. Owner and timestamp are server-authoritative (caller's citizenid + os.time()
---set here), so authorship and dates can't be spoofed. Capped at PG.MaxPostsPerPlayer active
---posts per character, and every field passes parseFields. The caller gets the canonical row
...
---Edit a post. Ownership-gated: the row's stored citizenid must equal the caller's, so a bare
...
---Delete a post. Ownership-gated like update. The id is normalised to a finite integer both for
---SQL hygiene and because the feed push echoes it back as a string - other phones match that
