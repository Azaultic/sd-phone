...
---just the most recent rows across all players. `price` is always NULL (kept only for schema
---parity with marketplace_listings - Pages is the priceless yellow-pages board) and `image` is
---an optional remote URL (same model as the Marketplace / Photos apps). Validation and ownership
---checks live in the actions layer - this layer stays dumb and every value is a ? parameter.
local store = {}
...
---Create the pages_posts table if it doesn't exist and back-fill newer columns, so the resource
---is drop-in: `email` (optional contact), then `images` (a JSON array of up to MaxImages URLs -
---`image` keeps the first one as the legacy column + card thumbnail so old rows still render).
---Run once at boot.
...
---Persist a new post. `price`/`image`/`images`/`email` may be nil (stored as SQL NULL).
...
---Update an existing post's editable fields in place. Ownership is checked by the caller
...
---Owner citizenid of a post, or nil if it doesn't exist - the ownership gate every mutation
