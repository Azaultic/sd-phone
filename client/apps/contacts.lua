...
-- number blocking - validation + persistence live in each server handler, documented there.
...
---Server push: a nearby player shared a contact card with us - relay it so the card is added
---to the list live. Server-originated, so the payload is trusted as-is.
...
---Server push: another resource removed our contacts matching a number - relay it so the rows
---drop from the list live. Server-originated, so the payload is trusted as-is.
